...
BEGIN
    ...
    IF NEW.status = 'confirmed' AND (OLD.status IS NULL OR OLD.status != 'confirmed') THEN
        ...
        -- ✅ NEW: Added expires_at and booking_end to metadata for auto-deletion
        ...
        VALUES (
            ...
            v_host_id,
            NEW.id,
            (v_booking_date + v_booking_time + v_duration) + INTERVAL '1 hour', -- ✅ Auto-delete 1 hour after booking ends
            jsonb_build_object(
                'booking_id', NEW.id,
                'venue', v_venue_name,
                'court', v_court_name,
                ...
                'booking_end', v_booking_date + v_booking_time + v_duration -- ✅ Store booking end time
            ),
            now(),
            now()
        )
...
-- Enable pg_cron if not already enabled (run this manually in SQL editor)
-- CREATE EXTENSION IF NOT EXISTS pg_cron;

-- Schedule cleanup to run every 5 minutes
-- Uncomment this after enabling pg_cron:
/*
SELECT cron.schedule(
  'delete-expired-game-chatrooms',           -- Job name
  '*/5 * * * *',                             -- Every 5 minutes
  $$SELECT delete_expired_game_chatrooms()$$ -- Function to run
);
*/
...
BEGIN
    ...
    IF NEW.status = 'confirmed' AND (OLD.status IS NULL OR OLD.status != 'confirmed') THEN
        ...
        -- Get booking details with venue and court info
        -- ✅ FIXED: Get sport type from court.type instead of v.sports[1]
        SELECT 
            v.name,
            c.name,
            b.booking_date,
            b.start_time,
            (b.end_time - b.start_time),
            c.type, -- ✅ Get sport type from court instead of venue
            b.user_id,
